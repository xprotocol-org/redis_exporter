...
func TestStreamsExtractStreamMetricsFromCluster(t *testing.T) {
	...
	e, _ := NewRedisExporter(
		...
		Options{
			Namespace:          "test",
			CheckSingleStreams: dbNumStrFull + "=" + TestKeyNameStream,
			IsCluster:          true,
		},
	)
	c, err := redis.DialURL(addr)
	...
	setupTestKeysCluster(t, addr)
	defer deleteTestKeysCluster(t, addr)
